...
Public Sub CalcBkKneeLoc(DataArray() As Single, ByVal TransitionSlope As Single, PositiveTransition As Boolean, ByVal TransitionPercentage As Single, ByVal TransitionWindow As Single, ByVal StartLoc As Single, ByVal StopLoc As Single, ByVal resolution As Single, KneeLoc As Single, KneeVal As Single, KneeFound As Boolean)
'
'   PURPOSE:    To calculate knee from the back of array.
'
'
...
For i = lintStop To lintStart Step -1

    'Calculate the point-to-point slope
    lsngPointToPointSlope = (DataArray(i) - DataArray(i - 1)) * resolution
    ...
    If PositiveTransition Then
        ...
        If lsngPointToPointSlope >= TransitionSlope Then
            ...
            lintNumBadPoints = 0
            For j = (i - 1) To (i - lintNumPointsInTransitionLength) Step -1
                'Calculate the point-to-point slope
                lsngPointToPointSlope = (DataArray(j) - DataArray(j - 1)) * resolution
